...
from issueandreturn import issueandreturn
    ...
    def issue_book(self, book_title, days=10):
        if days <= 10:
            for member in Member.members_list:
                if member == self:
                    book_item =issueandreturn.issue_book(self.name, self.student_id, book_title, days)
...
class Librarian(User):
    ...
    def addBook(self, title, author, publication_date,pages):
        Catalog.add_book(title, author, publication_date,pages)

    def addBookItem(self, title, isbn, rack):
        Catalog.add_book_item(title, isbn, rack)

    def removeBook(self, re_book):
        Catalog.remove_book(re_book)

    def removeBookItem(self, re_item):
        Catalog.remove_book_item(re_item)
